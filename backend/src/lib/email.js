...
const FROM       = 'Adoptly <bonjour@adoptly.fr>';
...
export async function sendWelcomeEmail({ email }) {
  if (!process.env.RESEND_API_KEY) {
    console.log('[Email] RESEND_API_KEY absent — email de bienvenue ignoré.');
    return;
  }
...
<body style="margin:0;padding:0;background:#F4F7FF;font-family:Inter,system-ui,sans-serif;">
  ...
  try {
    ...
    if (!res.ok) {
      ...
      console.error('[Email] Échec envoi email de bienvenue :', body);
    }
  } catch (err) {
