        ...
        <div className="flex items-center justify-between">
          ...
          {conversations.filter(c => c.unread > 0).length > 0 && (
            <span className="bg-red-500 text-white text-xs font-bold px-2.5 py-1 rounded-full">
              {conversations.reduce((sum, c) => sum + c.unread, 0)} non lu{conversations.reduce((sum, c) => sum + c.unread, 0) > 1 ? 's' : ''}
            </span>
          )}
        </div>
        ...
        {loading ? (
          ...
          <div className="card p-8 text-center space-y-3">
            ...
            <p className="text-gray-400 text-sm">
              Les messages des adoptants intéressés apparaîtront ici.
            </p>
          </div>
        ) : (
          <div className="space-y-2">
            {conversations.map((conv, i) => (
              <motion.button
                key={conv.match_id}
                onClick={() => navigate(`/shelter/chat/${conv.match_id}`)}
              ...
              >
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <p className={`text-sm mt-1 truncate ${conv.unread > 0 ? 'font-semibold text-gray-800' : 'text-gray-500'}`}>
                    {conv.last_sender === 'shelter' ? 'Vous : ' : ''}{conv.last_message}
                  </p>
                </div>
